...
public:
    ...
    bool isClipActive() const noexcept;

    struct FilterFrame
    {
        std::array<AuthenticEMUZPlane::BiquadCoeffs, 6> coeffs {};
        std::array<AuthenticEMUZPlane::PolePair, 6> poles {};
        float morph = 0.0f;
        float intensity = 0.0f;
    };

    bool getLatestFilterFrame (FilterFrame& dest) const;
...
private:
    ...
    static constexpr int maxSamplesPerBlock = 4096;
    ...
    juce::LinearSmoothedValue<float> smTrim;
    ...
    juce::dsp::ProcessorDuplicator<juce::dsp::IIR::Filter<float>, juce::dsp::IIR::Coefficients<float>> printLowpass;
    juce::dsp::ProcessorDuplicator<juce::dsp::IIR::Filter<float>, juce::dsp::IIR::Coefficients<float>> safeHighCut;
    std::vector<float*> oversampleChannelPointers;

    std::atomic<float> outputPeak { 0.0f };
    std::atomic<int> clipHold { 0 };
    bool lastSafeModeState = false;
    static constexpr int clipHoldFrames = 18;

    std::atomic<int> pendingStyleMacro { -1 };
    
    // Async quality update system
    struct QualityUpdateRequest
    {
        Quality quality;
        int blockSize;
        bool pending { false };
    };
    QualityUpdateRequest pendingQualityUpdate;
    juce::SpinLock qualityUpdateLock;
    ...
    void publishFilterFrame();

    std::array<FilterFrame, 2> coeffFrames {};
    std::atomic<uint32_t> coeffSequence { 0 };

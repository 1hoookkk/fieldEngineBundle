...
public:
    ...
    void getSectionCoeffs (std::array<BiquadCoeffs, 6>& dest) const;
...
private:
    ...
    std::vector<std::array<BiquadSection, 6>> channelStates;
    ...
    float processSampleInternal(float input, std::array<BiquadSection, 6>& sections);

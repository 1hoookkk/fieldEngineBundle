...
void AuthenticEMUZPlane::prepareToPlay (double sampleRate) {
    ...
    reset();
    makeupSm.reset(fs, 0.05);
    makeupSm.setCurrentAndTargetValue(1.0f);
    preRMSsq = postRMSsq = 1e-6f;
    updateCoefficientsBlock(64); // Prime the coefficients
}
...
void AuthenticEMUZPlane::process(juce::AudioBuffer<float>& buffer) {
    ...
    preRMSsq = 1e-6f; postRMSsq = 1e-6f;
    ...
    // Auto-makeup gain based on pre/post RMS
    if (autoMakeup) {
        const float pre  = std::sqrt(std::max(preRMSsq, 1e-12f));
        const float post = std::sqrt(std::max(postRMSsq, 1e-12f));
        float corr = post > 1e-9f ? pre / post : 1.0f;
        corr = juce::jlimit(0.5f, 2.0f, corr);
        makeupSm.setTargetValue(corr);
        for (int i = 0; i < nSm; ++i) {
            const float g = makeupSm.getNextValue();
            for (int ch = 0; ch < nCh; ++ch) buffer.getWritePointer(ch)[i] *= g;
        }
    }
}

void AuthenticEMUZPlane::updateCoefficientsBlock(int numSamples) {
    // Control-rate LFO advance with real block size
    if (lfoRate > 0.0f) {
        const float inc = juce::MathConstants<float>::twoPi * (lfoRate / fs);
        lfoPhase += inc * (float) numSamples;
    ...
    // Use raw parameters (plugin smooths externally)
    const float morph = juce::jlimit(0.0f, 1.0f, currentMorph + lfoUnipolar);
    const float I     = juce::jlimit(0.0f, 1.0f, currentIntensity);

...
public:
    ...
    void setClassicMode(bool classic) { classicMode = classic; }
    ...
    PitchBlock analyze(const float* in, int N) {
        ...
        float fTarget = 0.f;
        if (f0Smooth > 0.f) {
            const float nearest = (float) nearestNoteInScale(hzToMidi(f0Smooth), keyRoot, scaleMask, bias);
            if (classicMode) {
                // Classic Auto-Tune 5 style: direct snap to nearest note, optional retune smoothing
                if (retune01 >= 0.999f) {
                    // Effectively instant retune (hard snap)
                    fTarget = midiToHz(nearest);
                    lastMidi = nearest;  // Update immediately for next block
                } else {
                    // Apply retune speed smoothing to MIDI note transitions
                    fTarget = midiToHz(smoothToward(lastMidi, nearest, (float)N / (float)sr, retuneTauSeconds(retune01)));
                }
            } else {
                // Current Evo-style smoothing behavior
                fTarget = midiToHz(smoothToward(lastMidi, nearest, (float)N / (float)sr, retuneTauSeconds(retune01)));
            }
        }
...
private:
    ...
    bool classicMode = true;  // Default to classic mode (AT5-style snap)

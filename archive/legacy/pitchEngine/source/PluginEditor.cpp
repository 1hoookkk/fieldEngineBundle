...
: juce::AudioProcessorEditor (&p), proc (p)
{
    ...
    addAndMakeVisible(hardTuneButton);
    hardTuneButton.onClick = [this]
    {
        // Classic robotic tone = Follow; pass false to try “Lock” if you prefer
        proc.applyHardTunePreset(/*formantFollow=*/true);

        // Optional: make the “Track/lat ~ 0 ms” UI reflect reality immediately
        // latencyLabel.setText("lat ~ 0.0 ms", juce::dontSendNotification);
    };
    hardTuneButton.setClickingTogglesState(false);
    hardTuneButton.setColour(juce::TextButton::buttonColourId, juce::Colours::teal.withAlpha(0.35f));
...
void PitchEngineEditor::resized()
{
    ...
    auto topRow = getLocalBounds().reduced(12).removeFromTop(44);
    hardTuneButton.setBounds(topRow.removeFromRight(140).reduced(4));
...
void PitchEngineEditor::timerCallback()
{
    ...
    const float latMs = (float)proc.getLatencySamples() * 1000.0f / (float)proc.getSampleRate();

    header.setLatencyText(juce::String("lat ~ ") + juce::String(latMs, 1) + " ms");
    header.setStates (stateA, stateB, byp, print, secret);

    // Pull meters from processor atoms
    meters.setLevels (proc.meters.rmsL.load(), proc.meters.rmsR.load(),
                      proc.meters.clipL.load(), proc.meters.clipR.load());
}
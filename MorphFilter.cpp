...
float MorphFilter::processSample(float input, int channel)
{
    ...
    // Flush denormal values from state variables per DSP research
    constexpr float denormThreshold = 1.0e-20f;
    if (std::abs(state.z1) < denormThreshold) state.z1 = 0.0f;
    if (std::abs(state.z2) < denormThreshold) state.z2 = 0.0f;

#include <zplane/ZPlaneSpectralMask.h>
...
{
    DenormalFTZScope() noexcept = default;
    ~DenormalFTZScope() noexcept = default;
};
...
inline float interpAngle(float a0, float a1, float t) noexcept
{
    const float delta = wrapAngle(a1 - a0);
    return wrapAngle(a0 + t * delta);
}

inline float cubicHermite(float y0, float y1, float y2, float y3, float mu) noexcept
{
